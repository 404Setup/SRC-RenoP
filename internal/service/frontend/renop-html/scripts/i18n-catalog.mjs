...
const generatedLocaleCatalogPattern = /^catalog\.[A-Za-z]{2,3}(?:-[A-Za-z0-9]{2,8})+\.generated\.js$/;
...
export async function scanI18nCatalog({
    ...
    for (const entry of readdirSync(i18nDir, {withFileTypes: true})) {
        if (entry.isFile() && (entry.name === catalogName || generatedLocaleCatalogPattern.test(entry.name))) continue;
...
/**
 * Render one locale's statically aggregated dynamic-entry module.
 * @param {string} locale - Locale directory name.
 * @param {string[]} fragments - Validated fragment file names.
 * @returns {string} Generated module source.
 */
function renderLocaleCatalog(locale, fragments) {
    const identifiers = [];
    const imports = [];
    ...
    return [
        ...
        ...imports,
        '',
        `export default Object.freeze(Object.assign({}, ${identifiers.join(', ')}));`,
        '',
    ].join('\n');
}

/**
 * Render the validated lazy locale catalog module.
 * @param {string[]} locales - Sorted locale names.
 * @param {string[]} fragments - Canonical fragment names.
 * @param {string} referenceLocale - Locale kept in the initial application chunk.
 * @returns {string} Generated module source.
 */
function renderCatalog(locales, fragments, referenceLocale) {
    const imports = [];
    const referenceIdentifiers = [];
    for (const fragment of fragments) {
        const identifier = localeImportIdentifier(referenceLocale, fragment);
        referenceIdentifiers.push(identifier);
        imports.push(`import ${identifier} from './${referenceLocale}/${fragment}';`);
    }
    ...
    for (const locale of locales) {
        if (locale === referenceLocale) continue;
        loaderEntries.push(
            `    '${locale}': () => import('./catalog.${locale}.generated.js').then((module) => module.default),`,
        );
    }
    return [
        '/*',
        ...
        ...imports,
        '',
        `const defaultLocale = Object.freeze(Object.assign({}, ${referenceIdentifiers.join(', ')}));`,
        'const localeLoaders = Object.freeze({',
        ...loaderEntries,
        '});',
        '',
        `export const availableLocales = Object.freeze(${JSON.stringify(locales)});`,
        `const localePromises = new Map([['${referenceLocale}', Promise.resolve(defaultLocale)]]);`,
        '',
        '/**',
...
export async function generateI18nCatalog({
    ...
    const expectedLocaleCatalogs = new Set();
    for (const locale of scan.locales) {
        if (locale === referenceLocale) continue;
        const fileName = `catalog.${locale}.generated.js`;
        expectedLocaleCatalogs.add(fileName);
        const file = join(i18nDir, fileName);
        const source = renderLocaleCatalog(locale, scan.referenceFragments);
        if (!existsSync(file) || readFileSync(file, 'utf8') !== source) writeFileSync(file, source, 'utf8');
    }
    for (const entry of readdirSync(i18nDir, {withFileTypes: true})) {
        if (!entry.isFile() || !generatedLocaleCatalogPattern.test(entry.name)) continue;
        if (!expectedLocaleCatalogs.has(entry.name)) rmSync(join(i18nDir, entry.name));
    }
    console.log(
