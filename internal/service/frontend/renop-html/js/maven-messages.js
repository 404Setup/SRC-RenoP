 ...
 * @returns {{repository: string, domain: string, inviter: string, level: number}|null} Valid invitation payload.
 */
function mavenInvitationPayload(message) {
    ...
    return {repository, domain, inviter, level};
}
...
async function handleMavenInvitation(message, decision) {
    ...
    const endpoint = `/api/maven/repositories/${encodeURIComponent(payload.repository)}`
        + `/invitations/${encodeURIComponent(message.id)}/${decision}`;
    ...
    window.dispatchEvent(new CustomEvent('mavenMembershipChanged', {
        detail: {repository: payload.repository, domain: payload.domain}
    }));
    return true;
}
