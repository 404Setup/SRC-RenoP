...
export function renderRegistrationSettings(container, data, changed) {
    ...
    fields.appendChild(createToggleRow(t('registration.enabled'), '', data.enabled === true, checked => {
    ...
    for (const key of ['ip_interval', 'provider_cooldown']) {
        ...
        const amount = buildInput('number', data[key].value, '', event => {
            data[key].value = Number(event.target.value);
            changed();
        });
        amount.min = '1';
        amount.max = '525600';
        ...
        const unit = makeCustomSelect(['minute', 'hour', 'day', 'week', 'month'].map(value => ({
                value,
                label: t(`mail.${value}`)
            })),
            data[key].unit, value => {
