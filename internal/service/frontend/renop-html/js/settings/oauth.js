...
export function renderOAuthSettings(container, data, changed) {
    ...
    const presetSelect = makeCustomSelect(data.presets.map(value => ({value: value.type, label: value.name})), preset.type,
        value => { preset = data.presets.find(item => item.type === value); });
    ...
    add.addEventListener('click', () => {
        ...
        selected = {...structuredClone(preset), id, callback_url: window.location.origin + '/api/auth/oauth/' + id + '/callback'};
    ...
    function input(object, key, {type = 'text', max = 2048, hint = '', required = false, label = key} = {}) {
        const control = buildInput(type, object[key], '', event => { object[key] = event.target.value; changed(); });
    ...
    function render() {
        ...
        picker.replaceChildren(); editor.replaceChildren();
        if (!selected || !data.providers.includes(selected)) selected = data.providers[0];
        if (!selected) { editor.append(el('p', {class: 'cfg-hint'}, t('oauth.none'))); return; }
        const select = makeCustomSelect(data.providers.map((value, index) => ({value: String(index), label: value.name || value.id})),
            String(data.providers.indexOf(selected)), value => {
                selected = data.providers[Number(value)]; render(); picker.querySelector('button')?.focus();
            });
        ...
        editor.appendChild(createToggleRow(t('oauth.enabled'), '', selected.enabled === true,
            value => { selected.enabled = value; changed(); render(); editor.querySelector('renop-toggle input')?.focus(); }));
        ...
        input(selected, 'client_secret', {type: 'password', max: 4096, hint: t(selected.client_secret_configured ? 'oauth.secretSaved' : 'oauth.secretHint')});
        if (selected.client_secret_configured) editor.appendChild(createToggleRow(t('oauth.clearSecret'), '', selected.clear_client_secret === true,
            value => { selected.clear_client_secret = value; changed(); }));
        ...
        if (selected.type === 'stackexchange') {
            ...
            input(selected, 'api_key', {type: 'password', max: 4096, hint: t(selected.api_key_configured ? 'oauth.secretSaved' : 'oauth.keyHint')});
            if (selected.api_key_configured) editor.appendChild(createToggleRow(t('oauth.clearKey'), '', selected.clear_api_key === true,
                value => { selected.clear_api_key = value; changed(); }));
        }
        ...
        if (selected.type === 'custom') {
            for (const key of ['authorize_url', 'token_url', 'userinfo_url', 'issuer', 'jwks_url']) {
                input(selected, key, {type: 'url', required: selected.enabled && !['issuer', 'jwks_url'].includes(key),
                    hint: ['issuer', 'jwks_url'].includes(key) ? t('oauth.oidcHint') : ''});
            }
            const tokenAuth = makeCustomSelect(['client_secret_post', 'client_secret_basic', 'none'].map(value => ({value, label: value})),
                selected.token_auth, value => { selected.token_auth = value; changed(); });
            ...
            editor.appendChild(createToggleRow(t('oauth.disable_pkce'), t('oauth.pkceHint'), selected.disable_pkce === true,
                value => { selected.disable_pkce = value; changed(); }));
            selected.claims ||= {};
            for (const key of ['subject', 'username', 'name', 'email', 'email_verified', 'avatar']) {
                input(selected.claims, key, {max: 128, required: key === 'subject' && selected.enabled, label: 'claim_' + key, hint: t('oauth.claimHint')});
            }
        }
        ...
        remove.addEventListener('click', async () => {
            ...
            selected = data.providers[0]; changed(); render();
