...
test('registration gates confirmation, verifies email, imports optional profile, and clears private state', async () => {
    ...
    const context = vm.createContext({
        ...
        mailStatusLabel: status => 'mail.' + status, attachPasswordStrength: () => ({
            reset() {
            }
        }),
        ...
        fetch: async (url, options) => {
            ...
            if (url.endsWith('/status')) body = {enabled, email_required: mail};
                ...
                if (delayPending) await new Promise(resolve => {
                    release = resolve;
                });
                ...
                assert.equal(url, '/api/auth/registration');
                body = {username: 'alice'};
            }
    ...
    const submit = () => form.handlers.submit({
        preventDefault() {
        }
    });
    const fill = () => {
        field('registration-username').value = 'alice';
        field('registration-email').value = 'alice@example.com';
    ...
    context.updateRegistrationPage(true, true);
    await drain();
    fill();
    submit();
    await drain();
    ...
    fill();
    field('registration-send').handlers.click();
    field('registration-send').handlers.click();
    await drain();
    ...
    field('registration-code').value = '12345678';
    submit();
    submit();
    await drain();
    ...
    pending = {
        provider: 'github', email: 'verified@example.com', expires_at: Date.now() + 600000,
        username: 'occupied', username_available: false, nickname: 'Provider Name'
    };
    context.window.location.search = '?provider=github';
    delayPending = true;
    context.updateRegistrationPage(true, true);
    fill();
    submit();
    ...
    field('registration-import').checked = true;
    field('registration-import').handlers.change();
    ...
    assert.equal(timers.size, 0);
    pending = {
        provider: 'stackexchange', provider_name: 'Stack Exchange', email_required: true,
