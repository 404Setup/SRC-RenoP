...
test('private email aliases add through confirmation and remove without replacing the primary', async () => {
    ...
    const node = (tag, props = {}, ...children) => {
        const result = {tag, ...props, children, value: '', disabled: false, dataset: {}, handlers: {},
            replaceChildren(...next) { this.children = next; },
            addEventListener(type, fn) { this.handlers[type] = fn; },
            reportValidity() { return this.value.includes('@'); },
            querySelector() { return nodes.get('submit'); },
        };
    ...
    const context = vm.createContext({
        ...
        el: node, t: key => key, showAlert() {}, responseErrorMessage: async () => 'error',
        runButtonAction: async (button, action) => { if (!button.disabled) await action(); },
        apiRequest: async (path, options) => {
            requests.push({path, ...options});
            return {ok: true, json: async () => options.method === 'PUT' ? {id: 'job', ticket: 'private-ticket'} : next};
        },
    ...
    context.renderAccountEmailAliases({email_aliases: ['old@example.com'], email_verification_required: true}, update, assert.fail);
