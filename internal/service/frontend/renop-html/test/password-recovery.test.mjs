...
test('email recovery gates mail, submits once, polls privately, and clears stale page state', async () => {
    ...
    let timerID = 0, pagehide, enabled = false, status = 'queued', allowWeak = true, failConfirm = true,
        failPoll = false, delaySend = false, release;
    const field = id => {
    ...
    const context = vm.createContext({
        ...
        navigateToLogin: (path, options) => {
            navigation.push(['login', path, options.replace]);
        },
        logout: async reason => {
            navigation.push(['logout', reason]);
            context.updatePasswordRecoveryPage(false);
        },
        showAlert: () => {
        ...
        fetch: async (url, options) => {
            ...
            if (url.endsWith('/status')) body = {enabled};
                ...
                if (delaySend) await new Promise(resolve => {
                    release = resolve;
                });
    ...
    const send = () => form.handlers.submit({
        preventDefault() {
        }
    });
    const fill = () => {
    ...
    field('password-reset-confirmation').value = 'different';
    send();
    await drain();
