...
func TestGitHubOAuthExistingAccountAndSingleUseSession(t *testing.T) {
	...
	authorized, err := db.GetRecentGitHubPrincipal(identity.Username, "example-org", 0)
	require.NoError(t, err)
	...
	mfa, err := db.GetMFAState(identity.Username)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, http.StatusSeeOther, reauthStart.StatusCode)
	reauthURL, err := url.Parse(reauthStart.Header.Get("Location"))
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, "/account/login", secondFactorURL.Path)
	require.Equal(t, "1", secondFactorURL.Query().Get("mfa"))
	require.Equal(t, "/packages", secondFactorURL.Query().Get("return_to"))
	require.Len(t, secondFactor.Cookies(), 1)
	require.Equal(t, mfaCookieName, secondFactor.Cookies()[0].Name)
	require.NoError(t, secondFactor.Body.Close())
	sessions, err = db.ListUserSessions(identity.Username, "")
	require.NoError(t, err)
