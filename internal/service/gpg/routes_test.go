...
func TestProfileReleaseHistoryIsScopedToAuthenticatedUser(t *testing.T) {
	...
	request = httptest.NewRequest(http.MethodGet, "/api/auth/profile/gpg/releases?username=bob", nil)
	request.Header.Set("X-Test-User", "admin")
	request.Header.Set("X-Test-Manager", "true")
	response, err = app.Test(request)
	require.NoError(t, err)
	assert.Equal(t, http.StatusOK, response.StatusCode)
	...
	require.NoError(t, proto.Unmarshal(body, &history))
	require.Len(t, history.Releases, 1)
	assert.Equal(t, "org/example/demo-2.jar", history.Releases[0].ArtifactPath)

	request = httptest.NewRequest(http.MethodGet, "/api/auth/profile/gpg?username=bob", nil)
	request.Header.Set("X-Test-User", "admin")
	request.Header.Set("X-Test-Manager", "true")
	response, err = app.Test(request)
	require.NoError(t, err)
