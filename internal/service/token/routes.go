...
func SetupTokenRoutes(app fiber.Router, state *core.AppState, opChan chan<- TokenOp) {
	...
	api.Post("/:name/token", func(c fiber.Ctx) error { return GenerateTokenForUser(c, state, opChan) })
...
func GenerateTokenForUser(c fiber.Ctx, state *core.AppState, opChan chan<- TokenOp) error {
	name := strings.Clone(strings.ToLower(c.Params("name")))
	user := getUserFromCtx(c)
	if !RequireManager(user) {
		return c.Status(fiber.StatusForbidden).SendString("Forbidden")
	...
	audit.Log(state, &core.AuditLogEntry{
		...
		Details:    "New upload token generated by manager",
		AuthMethod: authMethod,
		SessionID:  sID,
		IP:         ip,
	})
