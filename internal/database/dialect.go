...
func initMavenTables(db *sql.DB, readmeColumnDefinition string) error {
	...
	tables := [...]string{
		`CREATE TABLE IF NOT EXISTS maven_domains (
			...
			health_checked_at BIGINT NOT NULL DEFAULT 0,
			health_next_check_at BIGINT NOT NULL DEFAULT 0,
			health_expires_at BIGINT NOT NULL DEFAULT 0,
			health_locked_at BIGINT NOT NULL DEFAULT 0,
			health_release_at BIGINT NOT NULL DEFAULT 0,
			health_lock_reason VARCHAR(16) NOT NULL DEFAULT '',
		...
		`CREATE TABLE IF NOT EXISTS maven_artifacts (
			...
			created_at BIGINT NOT NULL,
			updated_at BIGINT NOT NULL,
			reclaim_hold_at BIGINT NOT NULL DEFAULT 0,
...
var sharedIndexMigrations = []SchemaMigration{
	...
	{Name: "idx_tokens_email_retention", Query: "CREATE INDEX IF NOT EXISTS idx_tokens_email_retention ON tokens(email_released_at, deleted_at, name);"},
	{Name: "idx_tokens_audit_retention", Query: "CREATE INDEX IF NOT EXISTS idx_tokens_audit_retention ON tokens(audit_purged_at, deleted_at, name);"},
	{Name: "idx_package_deprecations_repository", Query: "CREATE INDEX IF NOT EXISTS idx_package_deprecations_repository ON package_deprecations(format, repository);"},
	{Name: "idx_resource_locks_package", Query: "CREATE INDEX IF NOT EXISTS idx_resource_locks_package ON resource_locks(package_id);"},
	{Name: "idx_resource_locks_repository", Query: "CREATE INDEX IF NOT EXISTS idx_resource_locks_repository ON resource_locks(repository);"},
	{Name: "idx_resource_locks_visibility", Query: "CREATE INDEX IF NOT EXISTS idx_resource_locks_visibility ON resource_locks(format, repository, mode, version);"},
	...
	{Name: "idx_audit_logs_operator", Query: "CREATE INDEX IF NOT EXISTS idx_audit_logs_operator ON audit_logs(operator);"},
...
var sharedColumnMigrations = []SchemaMigration{
	...
	{Name: "maven_domains.provider_type", Query: "ALTER TABLE maven_domains ADD COLUMN provider_type VARCHAR(16) NOT NULL DEFAULT '';"},
	{Name: "maven_domains.provider_id", Query: "ALTER TABLE maven_domains ADD COLUMN provider_id VARCHAR(32) NOT NULL DEFAULT '';"},
	{Name: "maven_domains.health_status", Query: "ALTER TABLE maven_domains ADD COLUMN health_status VARCHAR(16) NOT NULL DEFAULT '';"},
	{Name: "maven_domains.health_checked_at", Query: "ALTER TABLE maven_domains ADD COLUMN health_checked_at BIGINT NOT NULL DEFAULT 0;"},
	{Name: "maven_domains.health_next_check_at", Query: "ALTER TABLE maven_domains ADD COLUMN health_next_check_at BIGINT NOT NULL DEFAULT 0;"},
	{Name: "maven_domains.health_expires_at", Query: "ALTER TABLE maven_domains ADD COLUMN health_expires_at BIGINT NOT NULL DEFAULT 0;"},
	{Name: "maven_domains.health_locked_at", Query: "ALTER TABLE maven_domains ADD COLUMN health_locked_at BIGINT NOT NULL DEFAULT 0;"},
	{Name: "maven_domains.health_release_at", Query: "ALTER TABLE maven_domains ADD COLUMN health_release_at BIGINT NOT NULL DEFAULT 0;"},
	{Name: "maven_domains.health_lock_reason", Query: "ALTER TABLE maven_domains ADD COLUMN health_lock_reason VARCHAR(16) NOT NULL DEFAULT '';"},
	...
	{Name: "tokens.audit_purged_at", Query: "ALTER TABLE tokens ADD COLUMN audit_purged_at BIGINT NOT NULL DEFAULT 0;"},
