...
func TestNPMPackageLifecycleVisibilityAndTeamOwnership(t *testing.T) {
	...
	guestPackages, guestTotal, err := db.ListNPMPackages("npm", "guest", false, false, 20, 0)
	require.NoError(t, err)
	require.Len(t, guestPackages, 1)
	assert.Equal(t, 1, guestTotal)
	assert.Equal(t, "demo", guestPackages[0].Name)
	alicePackages, aliceTotal, err := db.ListNPMPackages("npm", "alice", false, false, 20, 0)
	require.NoError(t, err)
	...
	bobPackages, bobTotal, err := db.ListNPMPackages("npm", "bob", false, false, 20, 0)
	require.NoError(t, err)
...
func TestNPMPackageLatestVersionFallsBackWithoutDistTag(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "1.2.3", details.Package.LatestVersion)
	packages, total, err := db.ListNPMPackages("npm", "guest", false, false, 20, 0)
	require.NoError(t, err)
...
func TestPostgresNPMPackageIntegration(t *testing.T) {
	...
	guestPackages, guestTotal, err := db.ListNPMPackages("npm-pg", "guest", false, false, 20, 0)
	require.NoError(t, err)
	require.Empty(t, guestPackages)
	require.Zero(t, guestTotal)
	readerPackages, readerTotal, err := db.ListNPMPackages("npm-pg", "npm_pg_reader", false, false, 20, 0)
	require.NoError(t, err)
