...
func TestSuperTeamLifecycleLimitsAndImmutableMemberships(t *testing.T) {
	...
	require.NoError(t, db.SetSuperTeamMemberVisibility("platform-team", "bob", false))
	bobProfile, err := db.GetUserProfile("bob")
	require.NoError(t, err)
	visibleTeams, visibleTotal, err := db.ListVisibleUserSuperTeams(bobProfile.UserID, "", false, false, 10, 0)
	require.NoError(t, err)
	require.Empty(t, visibleTeams)
	require.Zero(t, visibleTotal)
	visibleTeams, visibleTotal, err = db.ListVisibleUserSuperTeams(bobProfile.UserID, "bob", false, false, 10, 0)
	require.NoError(t, err)
	require.Empty(t, visibleTeams)
	require.Zero(t, visibleTotal)
	visibleTeams, visibleTotal, err = db.ListVisibleUserSuperTeams(bobProfile.UserID, "alice", false, false, 10, 0)
	require.NoError(t, err)
	require.Len(t, visibleTeams, 1)
	require.Equal(t, 1, visibleTotal)
	require.False(t, visibleTeams[0].Visible)
	publicDetails, err := db.GetPublicSuperTeamDetails("platform-team", "", false, false)
	require.NoError(t, err)
	require.Len(t, publicDetails.Members, 1)
	require.Equal(t, "alice", publicDetails.Members[0].Username)

	details, err := db.GetSuperTeamDetails("platform-team", "bob", false, false)
	require.NoError(t, err)
	...
	details, err = db.GetSuperTeamDetails("platform-team", "alice", false, false)
	require.NoError(t, err)
	require.Len(t, details.Members, 2)
	require.NoError(t, db.SetSuperTeamMemberVisibility("platform-team", "alice", false))
	publicDetails, err = db.GetPublicSuperTeamDetails("platform-team", "", false, false)
	require.NoError(t, err)
	require.Empty(t, publicDetails.Members)
	require.Empty(t, publicDetails.Team.CreatedBy)
	require.NoError(t, db.SetSuperTeamMemberVisibility("platform-team", "alice", true))
...
func TestAccountDeletionPreservesSuperTeamOwnershipAndCreatorDisplay(t *testing.T) {
	...
	details, err := db.GetSuperTeamDetails("durable", "charlie", false, false)
	require.NoError(t, err)
