...
func TestSuperTeamLifecycleLimitsAndImmutableMemberships(t *testing.T) {
	...
	require.NoError(t, err)
	visibleTeams, visibleTotal, err := db.ListVisibleUserSuperTeams(bobProfile.UserID, "", false, 10, 0)
	require.NoError(t, err)
	require.Empty(t, visibleTeams)
	require.Zero(t, visibleTotal)
	visibleTeams, visibleTotal, err = db.ListVisibleUserSuperTeams(bobProfile.UserID, "bob", false, 10, 0)
	require.NoError(t, err)
	require.Empty(t, visibleTeams)
	require.Zero(t, visibleTotal)
	visibleTeams, visibleTotal, err = db.ListVisibleUserSuperTeams(bobProfile.UserID, "alice", false, 10, 0)
	require.NoError(t, err)
	require.Len(t, visibleTeams, 1)
	require.Equal(t, 1, visibleTotal)
	require.False(t, visibleTeams[0].Visible)
	publicDetails, err := db.GetPublicSuperTeamDetails("platform-team", "", false)
	require.NoError(t, err)
	...
	details, err = db.GetSuperTeamDetails("platform-team", "alice", false)
	require.NoError(t, err)
	require.Len(t, details.Members, 2)
