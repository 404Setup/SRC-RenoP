...
func GeneratePom(c fiber.Ctx, state *core.AppState) error {
	...
	releaseMutation := repositorygate.AcquireMutation(repoName)
	...
	metadataRelative, err := filepath.Rel(repoPath, metadataPath)
	if err != nil {
		return fiber.ErrBadRequest
	}
	if err := maven.EnsurePathMutable(state, repo, filepath.ToSlash(metadataRelative)); err != nil {
		if errors.Is(err, core.ErrResourceLocked) {
			c.Set("X-Renop-Error-Code", "resource_locked")
			return c.Status(fiber.StatusLocked).SendString("Maven resource is locked")
		}
