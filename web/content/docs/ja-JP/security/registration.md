...
他の設定済みサービスには[外部登録の規則](./oauth-login.md)が適用されます。コード発行と最終確認の両方にプロバイダー ID を送信してください。メールがない場合や未確認の場合は必ず RenoP のコードが必要で、メールが利用できなければ登録を完了できません。確認済みアドレスには追加コードは不要です。メール無効時にアドレスが任意になるのは手動登録のみです。
