...
`period` は `day`、`week`、`month`、`lifetime` のいずれかで、繰り返す期間の境界は UTC です。ゼロは所有者別設定でのみ使用でき、
該当する操作を禁止します。管理者専用の `unlimited` は、その所有者のクォータ消費を無効にします。空の
設定オブジェクトを送信すると、すべての全体既定値に戻ります。

長期（`lifetime`）クォータは使用量を累積し、自動的にはリセットされません。設定された上限は引き続き適用されます。
`period_start` と `period_end` は両方とも `0` です。定期クリーンアップやサーバー再起動でも累積値は保持されます。
繰り返す期間に切り替えても長期の累積値は消えず、長期に戻すと保存された合計から再開します。
繰り返す期間の使用量は、長期の合計に遡って加算されません。
