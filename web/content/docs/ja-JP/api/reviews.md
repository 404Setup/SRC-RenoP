...
レビュー担当ビューには、アカウントが T3/T4 であるチームの移管と、モデレーターを担当するリポジトリの
公開審査が表示されます。システム管理者はすべてのタスクを確認できます。申請者ビューは不変のアカウント
ID を使うため、ユーザー名変更前の申請も維持されます。

新しいタスクは、対象チームの審査担当者、リポジトリモデレーター、システム管理者へ重複しないメッセージを
送ります。最初の決定後は残りの担当者通知を削除し、申請者へ承認、拒否、取消のローカライズ済み結果を
送ります。申請者と管理者ではないモデレーターには `decided_by` を返さず、決定者を確認できるのは
システム管理者だけです。
...
ストリーミングします。申請者、担当リポジトリモデレーター、システム管理者のブラウザーセッションだけが利用できます。
...
POST /api/reviews/{id}/decision は `approved` または `rejected` を受け付けます。移管の拒否には 512 文字以内の
