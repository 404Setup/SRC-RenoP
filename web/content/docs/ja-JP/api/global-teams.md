...
GET /api/super-teams/eligible は、呼び出し元が T3 以上を持つチームだけを返します。スラッシュを含む Docker
イメージは先頭のパス要素と一致するチームを選択し、スコープ付き npm パッケージは `@` を除いたスコープと一致する
チームを選択します。プレフィックスのない名前は個人所有のまま利用できます。
