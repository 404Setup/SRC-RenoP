...
Docker の作成画面は T2 を指定し、対象メンバーが審査を申請できるようにします。スラッシュを含む Docker
イメージは先頭要素と一致するチームを選択し、スコープ付き npm
...
L4 所有者は `/account/tickets` で申請し、審査チームの T3/T4 管理者またはシステム管理者が一度だけ決定します。
...
システム管理者とグローバルモデレーター（`canmoderate:*`）は、有効なブラウザーセッション Cookie でチームをロックできます。リポジトリ限定のモデレーター権限ではグローバルチームを変更できず、API トークンではロックを管理できません。
...
両モードとも、チーム情報、メンバー変更、招待の承諾、クォータ上書き、所有権移譲、関連リソースの作成と変更を凍結します。`write` はダウンロードを維持します。`read` は非メンバーからチームを隠し、権限を持つスタッフと既存の所有者・共同作業者（T1 を含む）だけがパッケージのメタデータを閲覧できます。ファイル内容は全員が取得できません。メンバー一覧と役割は保持されます。Maven ドメインの所属チームのロックは未登録パスにも適用され、独立して検証された子ドメインは自身の権限を維持します。

チーム一覧と詳細は公開の `locks` を返します。詳細の `moderator` は現在のリクエストの権限を示し、チーム管理権限を追加しません。アカウント画面と公開画面に翻訳された理由とスタッフ操作を表示します。DELETE は `{}` を受け取り手動ロックのみ解除し、システムロックは独立して残ります。書き込み拒否は `423` と `X-Renop-Error-Code: resource_locked`、非表示リソースは `404` を返します。解除後は保持された権限が復元されますが、他の有効なロックは適用されます。
