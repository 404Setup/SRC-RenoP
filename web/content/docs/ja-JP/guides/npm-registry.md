...
どちらの公開審査方針でも package 作成は `202 Accepted` を返し、承認まで名前を予約しません。
`new_packages` では後続の `npm publish` は通常どおり実行されます。`every_version` では各 publish も審査対象となり、
承認まで packument と tarball 経路には現れません。リポジトリモデレーターまたはシステム管理者が不変の
manifest、dist-tag、tarball をまとめて審査します。
...
パッケージとバージョンのページに、翻訳された公開のロック理由を表示します。管理者と対象リポジトリのモデレーターは
これらのページで手動ロックを管理できます。書き込みロックは変更を停止し、読み取りロックはメタデータの閲覧を
管理担当者と現在の協力者に限定して、すべてのファイル取得を禁止します。手動ロックを解除してもシステムの制限は残ります。

ロック済みバージョンは関連するタグを保持し、パッケージ全体のライフサイクル変更を禁止します。既存タグがロック済みの
バージョンを指す場合は、別のタグで新しいバージョンを公開してください。ミラー更新はロック済みの保存内容を保持します。
エンドポイントと応答コードは [npm API](/docs/api/npm) を参照してください。

移動前のパッケージ応答やロック更新の応答は破棄し、別のパッケージページを上書きしません。
