...
**Другие провайдеры OAuth**: `GET /api/settings/oauth-providers` возвращает клиенты и шаблоны без секретов;
`PUT /api/settings/oauth-providers` заменяет список. Массив `providers` обязателен; явно пустой массив удаляет все
настроенные клиенты. Допускаются до 32 клиентов и тело до 128 KiB. Учётные данные, шаблоны и привязки описаны в
разделе [Вход через сторонние сервисы](../security/oauth-login.md).
