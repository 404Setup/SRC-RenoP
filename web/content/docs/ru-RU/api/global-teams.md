...
GET /api/super-teams/eligible возвращает только команды, где вызывающий имеет уровень не ниже T3. Образ Docker с
косой чертой должен выбрать команду, совпадающую с первым компонентом пути. Пакет npm с областью должен выбрать
команду, совпадающую с областью без `@`. Имена без префикса могут оставаться личными.
