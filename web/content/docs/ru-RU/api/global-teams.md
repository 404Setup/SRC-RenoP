...
GET /api/super-teams/eligible по умолчанию возвращает команды с уровнем не ниже T3; для выбора цели передачи
`minimum_role` принимает T1-T4. Образ Docker с косой чертой должен выбрать команду первого компонента пути. Пакет npm
с областью должен выбрать команду области без `@`. Имена без префикса могут оставаться личными.
