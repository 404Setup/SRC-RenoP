...
Ответ с подробностями пакета содержит ограниченные по размеру README, сведения об авторе, участниках и сопровождающих,
лицензию, требования к среде, ключевые слова и ссылки проекта из выбранной опубликованной версии. Браузер отображает
Markdown README через список разрешённых элементов и URL, не активируя HTML и опасные ссылки из пакета.
...
Администраторы и модераторы репозитория управляют блокировками через действующий cookie браузерной сессии:
...
Для всего пакета опустите `version` или передайте `""`. Режимы: `write` и `read`. Публичные причины переводятся:
`hold`, `prohibited`, `expired`, `trojan`, `abuse`, `dmca`, `reup`, `squatting` и `quality`.
DELETE принимает `{"version":"1.2.3"}` и снимает только указанную ручную блокировку. Системные блокировки независимы.
API-токены не могут управлять блокировками. Сведения о пакетах и версиях содержат публичные записи `locks`.

Оба режима запрещают изменения. Блокировка чтения также оставляет метаданные только администраторам, модераторам
репозитория, владельцам и участникам, включая L0 и участников связанной глобальной команды. Загрузка файлов запрещена
всем. Остальные посетители не видят ограниченные версии и их теги в полных и сокращённых packument, поиске, профилях
и ресурсах команд. Для отфильтрованных метаданных кеш отключён; устаревший ответ `304` не возвращается.

Блокировка версии запрещает изменения связанных dist-tag, а также архивирование, окончательное прекращение поддержки
и удаление всего пакета. Полный packument может повторять неизменённые заблокированные метаданные.
Другие версии можно публиковать с тегами, которые не переносят тег заблокированной версии.
Любая блокировка останавливает замену packument из зеркала; затронутые tarball в кеше не истекают и не загружаются заново.
Заблокированный репозиторий нельзя перенастроить или удалить.

Отклонённые изменения возвращают `423` и `X-Renop-Error-Code: resource_locked`; отклонённое чтение возвращает `404`.
