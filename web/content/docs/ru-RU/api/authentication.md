...
Начальная конфигурация учётных записей и токенов может передаваться через `tokens.yaml` (`RENOP_TOKENS`). При запуске
процесса данные автоматически мигрируют и сохраняются во встроенную базу данных SQLite (по умолчанию `renop.db`).
Права — список строк.
...
Идентификатор сессии выставляется только в cookie `renop_session`. Поле `session_token` в ответе login пустое; браузеры
используют cookie, скрипты могут передать тот же id как `Authorization: Session …`.
...
Список **браузерных сеансов входа** текущего пользователя. Basic и Bearer **не** создают сеансы и здесь не появляются.
Секрет сеанса (значение cookie) **никогда** не возвращается.
...
Отзывает все браузерные сеансы текущего пользователя **кроме** сеанса этого запроса. Ответ: `StatusOk` protobuf
(`status: success`).
...
Удалить **одну из своих** сессий по `public_id`. Ответ: `StatusOk` protobuf. Отсутствующий id — no-op. Отзыв текущего
сеанса очищает cookie.

## Управление сеансами менеджером

Менеджеры (`admin` / `manager`) могут просматривать и отзывать браузерные сеансы **любой** учётной записи через
`/api/tokens`.
