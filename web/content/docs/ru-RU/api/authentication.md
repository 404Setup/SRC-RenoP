...
Страница входа в браузере находится по адресу `/account/login`. После входа с паролем, Passkey или сторонним провайдером пользователь возвращается к локальному пути из необязательного параметра `return_to`; строка запроса и фрагмент не сохраняются. Внешние адреса, конечные точки аутентификации и значения длиннее 1 024 символов заменяются на `/`. При истечении сеанса открывается страница входа; при отказе в доступе авторизованный пользователь возвращается на главную страницу без завершения сеанса.
...
Непривязанные идентичности GitHub требуют [регистрации](../security/registration.md) и пароля. OAuth запрашивает `read:user read:org user:email`; обратный вызов требует cookie браузера, начавшего авторизацию.

Microsoft, Google, GitLab, Cloudflare, Stack Exchange и собственные клиенты OAuth используют [API стороннего входа](../security/oauth-login.md). Доступны привязки аккаунтов, обязательная при необходимости проверка почты при регистрации, необязательный импорт профиля и та же политика второго фактора.
...
- Отключить пароль можно только при наличии основного Passkey или стороннего аккаунта. Для включения пароль должен быть задан.

### Сброс пароля по коду из письма

Отдельная страница `/account/forgot-password` проверяет личный адрес, сохранённый в аккаунте. При отключённой почте страница и API запроса и подтверждения возвращают `404`, а страницы входа и восстановления скрывают ссылки. Офлайн-восстановление `/account/recovery` остаётся доступным.

- **Доступность**: `GET /api/auth/password-reset/status` возвращает `{"enabled":true}` или `{"enabled":false}`.
- **Отправка кода**: `POST /api/auth/password-reset/request` принимает `{"email":"admin@example.com"}` и возвращает `202` с `{id,status,ticket}` после надёжной записи в очередь, до отправки.
- **Доставка**: `GET /api/auth/mail/:id` принимает закрытый билет в заголовке `X-Renop-Mail-Ticket`. Страница проверяет состояние каждые 3 секунды не более 10 минут и прекращает проверку при уходе. Ошибки и неопределённый результат показываются пользователю. Принятие провайдером не гарантирует доставку получателю. Не сохраняйте билеты в URL или хранилище браузера.
- **Сброс**: `POST /api/auth/password-reset/confirm` принимает JSON ниже. Оба POST требуют `Content-Type: application/json`, тело не более 4 096 байт и пароль длиной 6–72 байта UTF-8.

```json
{"email":"admin@example.com","code":"01234567","new_password":"new_secure_password"}
```

Восьмизначный код действует 10 минут и допускает пять ошибок. Помимо настроенного ограничения IP, для адреса действует задержка 60 секунд между выдачами. Новый код отменяет предыдущий только после успешной записи в очередь; при ошибке старый сохраняется. Запись письма, кода и счётчика лимита выполняется одной транзакцией. Хранится не более 2 048 подтверждений; просроченные записи удаляются перед добавлением и периодически.

Все допустимые адреса, разрешённые политикой получателей, получают одинаковое письмо проверки владения, включая незарегистрированные: состояние доставки не раскрывает наличие аккаунта. При сбросе повторно проверяются идентификатор, почта, пароль и версия безопасности на момент выдачи. Аккаунт, зарегистрированный позднее, не может использовать старый код; закрытие аккаунта также делает коды недействительными.

Успех атомарно расходует код, меняет пароль, включает вход по паролю и отзывает браузерные сессии. Ответ `{status:"success",username}` очищает cookie и возвращает страницу ко входу; другие способы входа остаются связанными, блокировки сохраняются. `ACCOUNT_EMAIL_CODE_INVALID` означает неверный, просроченный, исчерпанный, использованный или устаревший код. Лимиты возвращают `429`, ошибки очереди или сервиса — `503`. Все ответы используют `Cache-Control: no-store`.

### Коды восстановления

Отдельная страница восстановления аккаунта `/account/recovery` доступна по ссылке восстановления на странице входа и работает без отправки почты. После восстановления браузер очищает прежнюю сессию и возвращается ко входу с заполненным именем пользователя, сохраняя допустимый локальный адрес `return_to`. Коды восстановления и пароли очищаются при уходе со страницы и не сохраняются в истории браузера.
...
Закрытие навсегда резервирует учётную запись и имя, удаляет членство в командах, освобождает все сторонние привязки входа
