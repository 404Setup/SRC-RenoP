...
Страница входа в браузере находится по адресу `/account/login`. После входа с паролем, Passkey или GitHub пользователь возвращается к локальному пути из необязательного параметра `return_to`; строка запроса и фрагмент не сохраняются. Внешние адреса, конечные точки аутентификации и значения длиннее 1 024 символов заменяются на `/`. При истечении сеанса открывается страница входа; при отказе в доступе авторизованный пользователь возвращается на главную страницу без завершения сеанса.
