...
На странице пакета администраторы и модераторы репозитория могут заблокировать пакет или отдельную версию,
выбрав публичную причину. Блокировка записи запрещает изменения. Блокировка чтения также оставляет метаданные
доступными только сотрудникам и участникам и запрещает всем скачивание файлов и просмотр Cargodoc.
Участники сохраняют доступ к метаданным, но не могут менять заблокированное содержимое. Снятие ручной блокировки
не отменяет системную. Другие незаблокированные версии можно публиковать, но операции со всем пакетом запрещены.

Запросы, причины и коды ответов описаны в [Cargo API](/docs/api/cargo).
