...
**manager** / **admin** 可在「设置」「仓库」里改大部分项。部分改文件后要重载/重启（见各配置域）。

## 存储

- **本地磁盘**（`storage_path`，默认）
- **S3 兼容**对象存储（`repositories.yaml` 里按仓库配）

上传可写 MD5 / SHA-1 / SHA-256 / SHA-512 旁路校验文件。

可见性、镜像、S3 字段见 [仓库与镜像](./repositories.md)。
