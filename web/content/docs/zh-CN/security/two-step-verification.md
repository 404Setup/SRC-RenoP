...
Passkey 登录、二次验证和注册会在操作按钮上显示倒计时。每次系统验证提示最多等待 60 秒，若服务器要求更短的时间则采用该时间。超时会取消提示并结束本次登录，包括待完成的二次验证挑战；迟到的凭据会被丢弃。请重新开始登录以重试。离开页面也会取消未完成的提示。取消、凭据不可用或被拒绝、环境不支持或不安全、重复注册以及无法满足验证要求等情况均显示本地化提示，不会暴露浏览器异常。
