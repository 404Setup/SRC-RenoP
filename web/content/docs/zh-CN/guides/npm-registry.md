...
先创建格式为 `npm` 的仓库，再从仓库页面预留每个软件包，然后才可发布。RenoP 不允许客户端在发布时
隐式创建软件包名称。以下示例使用 `javascript` 仓库和 `@example/library` 软件包。

## 配置客户端

创建具有仓库读取与发布权限的可过期 API Token。只有自动化流程确实需要时，才授予软件包生命周期或团队
管理权限。若此仓库用于全部软件包，可在项目或用户级 `.npmrc` 中写入：
...
若只需将一个作用域交给 RenoP，请保留默认仓库并单独配置该作用域：
...
路由中。仓库版主或系统管理员会一并审核不可变 manifest、dist-tag 与 tarball。

## 可见性与软件包团队

公开软件包遵循仓库可见性。私有软件包必须使用作用域，并要求明确的软件包成员关系或管理员权限。L0 可读取，
...
npm 仓库可按顺序代理上游仓库。可用精确软件包名称和 `@scope/*` 规则约束镜像范围。RenoP 会限制 packument
大小、合并并发刷新、将上游 tarball 地址替换为本地地址，并从本地目录移除上游已删除的版本。通过镜像发现的软件包
不能接受本地推送。

## 资源锁定

包与版本页面显示本地化的公开锁定原因，管理员和对应仓库的版主可在页面中管理人工锁定。
禁止写入会冻结修改；禁止读取还会将元数据限制为工作人员和当前协作者可见，并禁止所有文件下载。
移除人工锁定不会解除仍有效的系统限制。

锁定的版本会保留关联标签，并阻止整个包的生命周期操作。如果现有标签指向锁定版本，
发布其他版本时需使用独立标签。镜像刷新会保留被锁定的已存储内容。
接口和响应码见 [npm API](/docs/api/npm)。

页面跳转后会丢弃旧的包数据及锁定刷新响应，避免覆盖新包的页面状态。
