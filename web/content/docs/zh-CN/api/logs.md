...
单个消费者串行写入行为和系统记录，正常关闭时恢复进程日志输出并排空队列。队列上限为 500
条；满时行为日志继续使用现有同步写入回退，进程日志保留在原输出中、增加失败计数，并在恢复容量后记录队列溢出。写入失败仅输出到原进程日志，避免递归。行为日志沿用
`audit_log` 的期限与数量上限；系统日志独立计算，最多保留 30 天、10000
条，若配置更低则采用较低值。每十分清理一次。账号注销保留期与永久清除规则也适用于关联系统记录。不会导入历史进程日志。
