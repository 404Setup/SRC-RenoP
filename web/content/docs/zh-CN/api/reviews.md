...
待我审核视图包含当前账号以 T3/T4 身份管理的团队转让与 T2 软件包创建申请。完成必要的团队阶段后，存储库发布
任务才会显示给对应版主。系统管理员可以查看所有任务。我的申请视图按照不可变账号标识返回，因此修改用户名不会丢失原有记录。

新任务会向当前阶段的审核人和系统管理员发送去重提醒。T2 创建任务转入存储库阶段时，系统会删除团队提醒并创建
版主提醒，此时不会向申请人发送已批准结果。最终处理会删除其余审核提醒并向申请人发送本地化结果。申请人与
非系统管理员版主不会收到 `decided_by`；只有系统管理员可以查看最终审核人。
...
npm 的 T2 成员始终先等待团队 T3/T4 批准，且不占用名称。若存储库启用了创建审核，同一任务会继续转交版主；
否则团队批准会原子创建软件包。最终批准会重新检查存储库权限与当前团队成员资格，再向申请人授予 L4。
...
T2 创建申请获得团队批准后会清除此字段，并以 `pending` 状态继续等待存储库审核。
...
标记。GET /api/reviews/{id}/files/{file_id} 流式返回一个被隐藏的文件。只有申请人、当前审核团队 T3/T4、完成团队
阶段后的对应存储库版主或系统管理员使用浏览器会话时可以访问。
...
POST /api/reviews/{id}/decision 接受 `approved` 或 `rejected`。批准 T2 创建申请时，若仍需存储库审核，会返回同一
