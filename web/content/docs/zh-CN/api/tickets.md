...
工单中心统一承载用户反馈、功能建议、违规举报、所有权转让及发布审批等事务性流程。

## 适用范围与凭据

所有工单接口均需在有效浏览器会话（`renop_session` Cookie）下调用，不支持使用 Basic 凭据或 API Token。
管理页面入口为 `/account/tickets`。

仓库版主可查看所管辖范围内的工单；系统管理员拥有全量查看权限。
T3/T4 团队成员负责处理涉及本团队的转让与创建流程。申请记录内部关联不可变账号 ID。

工单处理流程采用严格的隐私保护与回避机制：申请人无法查看具体处理人员的身份字段；
被举报账号即使拥有管理员权限，亦无法查看或处理针对自身的举报，确保处理的公正性与匿名性。

待处理提醒根据当前审批阶段自动推送。最终结果通过系统消息及邮件通知申请人；
驳回或撤回不通知被举报对象。举报审计事件不记录敏感身份信息。
