...
Docker 映像，此类申请必须由 T3/T4 成员批准；T3 可管理 T1/T2 成员并直接创建软件包；T4 拥有团队配置，并可授予 T3/T4。
...
创建界面会请求 T2，使符合条件的成员能够提交审核任务。名称中含斜杠的 Docker 映像必须选择与第一段路径一致的
团队；带作用域的 npm 包必须选择与去掉 `@` 后的作用域一致的团队。无前缀 Docker 映像与无作用域 npm 包仍可归个人所有。

T2 创建申请不会预占软件包名称。T3/T4 团队批准是第一阶段；若存储库还启用了新包审核，同一任务随后转交存储库
版主，名称在最终批准前始终可用。该顺序保证团队权限与存储库审核都不能绕过对方。

Cargo crate、Maven 制品和 Maven 发布域使用同一绑定。最终权限取软件包显式权限与团队映射权限中的较高值，团队成员
不会被复制到软件包成员表。团队仍拥有项目或发布域时不可删除，必须先完成转让或移除。
L4 所有者在 `/account/reviews` 提交转让，由审核团队 T3/T4 管理员或系统管理员进行一次性处理。
