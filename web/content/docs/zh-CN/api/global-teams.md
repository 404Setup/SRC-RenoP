...
GET /api/super-teams/eligible 默认返回当前账户至少拥有 T3 的团队；转让选择器可通过 `minimum_role` 请求 T1-T4。
名称中含斜杠的 Docker 映像必须选择与第一段路径一致的团队；带作用域的 npm 包必须选择与去掉 `@` 后的作用域一致
的团队。无前缀 Docker 映像与无作用域 npm 包仍可归个人所有。
