...
制品详情响应汇总已索引的主文件、大小、修改时间以及校验和与 GPG 签名覆盖情况。
系统自动解析 POM 元数据，提取项目名、许可证、开发者与直接依赖信息供详情页展示。

L2–L4 域成员或管理员可通过制品更新接口维护独立的 Markdown 说明文档（上限 512 KiB）。
POM 或目录中的简要描述仍保持为独立字段。
...
包页面为管理员和 L3/L4 协作者提供管理发布域团队入口，打开现有发布域团队页。成员变更适用于该域下所有包，包括旧仓库导入的包。长坐标与操作按钮分别换行，制品和最新 POM 信息使用完整内容宽度，依赖格式支持动画切换与键盘操作。

锁定窗口为当前资源自身的锁提供直接解除手动锁定操作。继承限制需要在发布域或全局团队中修改，系统安全锁仍遵循已有的赎回与恢复流程。

Maven 列表和详情均返回包的锁定状态，包括继承限制。界面显示锁定标记，并为当前资源的手动锁提供明确的解锁入口。未锁定表示移除此手动锁，不会移除上级或系统限制。

所有锁定 API 都支持 `reason: custom`，并要求非空的 `reason_text`，最多 256 个字符。可查看资源的用户可以看到这些纯文本，内容不得包含控制字符。使用预置原因时省略 `reason_text`。
