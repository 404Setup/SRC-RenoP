...
`period` accepte `day`, `week`, `month` ou `lifetime`. Les périodes récurrentes utilisent UTC. Une valeur nulle n'est
admise que dans une règle
...
Les quotas à long terme (`lifetime`) cumulent l’utilisation sans réinitialisation automatique et appliquent les limites
configurées.
Leurs champs `period_start` et `period_end` valent `0`. Le nettoyage périodique et les redémarrages conservent cette
utilisation.
